...
def update_grid(grid_size, cells, p_grid, m_grid, p_node_capacity, m_node_capacity, pp, migration_prob, alpha, a_pm, b_pm, theta_pm, k_pm, a_mp, b_mp, theta_mp, k_mp, bm_slice):
    ...
    cells.extend(new_cells) # lägger till de nya cellerna i "cells"
    return cells, p_grid, m_grid


def simulate(pp, migration_prob, alpha, a_pm, b_pm, theta_pm, k_pm, a_mp, b_mp, theta_mp, k_mp):
    ...
    for t in range(time_steps):
        cells, p_grid, m_grid = update_grid(grid_size, cells, p_grid, m_grid, p_node_capacity, m_node_capacity, pp, migration_prob, alpha, a_pm, b_pm, theta_pm, k_pm, a_mp, b_mp, theta_mp, k_mp,bm_slice)

    occupancy = (np.count_nonzero(p_grid, axis=2))
