...
prior = pyabc.Distribution(pp=pyabc.RV("truncnorm",pp_a, pp_b, loc=pp_mu, scale=pp_sigma),
                           wm=pyabc.RV("truncnorm",wm_a, wm_b, loc=wm_mu, scale=wm_sigma),
                           gm = pyabc.RV("truncnorm", gm_a, gm_b, loc=gm_mu, scale=gm_sigma),
...
def model(parameter):
    try:
        ...
        result = simulate(
            cells, grid_size,
            parameter["pp"], parameter["wm"], parameter["gm"],
...
epsilon_schedule = pyabc.QuantileEpsilon(initial_epsilon=0.4, alpha= 0.6, quantile_multiplier=1)
sampler = pyabc.sampler.MulticoreEvalParallelSampler()
abc_cont = pyabc.ABCSMC(model, prior, distance, population_size=10 ,eps = epsilon_schedule ,sampler=sampler, acceptor=acceptor)
